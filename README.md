...
**Quick Note on Portability** *This SD Card module is intended to work with the SPI port on an **ATMega1280 AVR microcontroller**. Any of the AVR-specific functionality, however, is handled entirely within the AVR IO port access files found under ***avrio*** within this repo. These IO files, **AVR_SPI** and **AVR_USART**, are not considered part of the SD card module, but are maintained in [AVR-IO](https://github.com/Jsfain/AVR-IO). As such, it should be straightforward to implement the SD Card module to operate against other target devices by ensuring the few SPI- and USART-specific macros and functions required in this module are included where needed. **AVR_SPI** is included by the SD_SPI_BASE files and **AVR_USART** is included by the PRINTS helper file and is only necessary if using any of the printing functions. See the SD_SPI_BASE and the PRINTS files for specific details.
